...
build () {
    ...
    cargo +nightly build --features run_integration_tests -Z build-std=core,alloc --target powerpc-unknown-eabi.json
    cp /build/target/powerpc-unknown-eabi/debug/rust-wii.elf /build/bin/boot-test.elf

    # TEST_EXECUTABLE=$(cargo +nightly test --no-run -Z build-std=core,alloc -Z panic-abort-tests --target powerpc-unknown-eabi.json | jq --join-output 'select(.reason=="compiler-artifact") | .executable')
    # cp $TEST_EXECUTABLE /build/bin/boot-test.elf
    echo -e "\e[1;32m Binary test-build completed. \e[0m"
}
